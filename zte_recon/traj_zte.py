...
from .arc_zte_traj import (
    get_segment_waveform_from_kacq_file,
    rotate_integrate_all_segments,
    read_params_from_kacq,
)
...
class Traj_ZTE:
    ...
    def setup_waspi_spokes(self):
        ...
        """
        self.coord_waspi = self.coord_full[
            0 : self.nInSpokes, 0 : (self.highMerge + 4) * self.waspi_scale, :
        ]
...
class Traj_CalcPhyllo_ZTE(Traj_ZTE):
    ...
    def __init__(
        ...
        if num_segs is None or num_segs_lowres is None:
            raise ValueError(
                "Need to provide both num_segs and num_segs_lowres to calculate Phyllo traj"
            )
        else:
    ...
    def get_coords(self):
        ...
        endpoints_hires = phyllo_endpoints_merlin(
            spokes_per_seg=self.spokes_per_seg * self.segsPerInterleaf,
            smoothness=self.smoothness,
            spokes_total=self.nSpokes,
            gm=self.gm_flag,
        )
        endpoints_waspi = phyllo_endpoints_merlin(
            spokes_per_seg=self.spokes_per_seg * self.segsPerInterleaf,
            smoothness=self.smoothness,
            spokes_total=self.nInSpokes,
            gm=self.gm_flag,
        )
...
class Traj_RadialZTE_from_kacq(Traj_ZTE):
    ...
    def __init__(
        self, opxres, endpoints_txt_path, spokes_per_seg, num_segs_lowres, waspi_scale=8
    ):
...
class Traj_ArcZTE_from_kacq(Traj_ZTE):
    ...
    def get_coords_using_kacq_file(self):
        ...
        self.spokes_all = rotate_integrate_all_segments(
            self.grad_first_seg,
            self.seg_rot_file,
            self.points_per_spoke,
            self.points_before_curve,
            self.dt_sampling,
            self.num_segs,
            self.num_segs_lowres,
            self.spokes_per_seg,
            self.waspi_scale,
            self.opxres,
            self.grad_dt_sampling,
        )

        ## Calculate correct scaling for reconstructing correcting FOV
        self.coord_full = np.copy(
            self.spokes_all[:, 0 : self.opxres]
        )  # only sampling part of gradients

        acquired_res = (
            self.opxres
        )  # TODO: check that this is giving the correct resolution
        scale_factor_fov = (
            acquired_res * 1 / (2 * self.coord_full.max())
        )  # digitize acquired resolution
...
class Traj_Calc_ArcZTE(Traj_ZTE):
    ...
    def setup_all_curved_grads(self):
        ...
        spoke_arr = np.zeros(
            (self.spokes_per_seg, 3, self.points_per_spoke - self.points_before_curve)
        )  # one segment of spokes
        ...
        first_spoke = single_spoke_arc(
            self.points_per_spoke + 1, angle=self.arc_angle, r=r
        )
        first_grad = np.diff(first_spoke) / (self.dt_sampling * 0.001 * gamma_bar)

        grad_cat[0] = first_grad
        spoke_arr[0] = (
            np.cumsum(grad_cat[0, :, self.points_before_curve :], axis=-1)
            * gamma_bar
            * self.dt_sampling
            * 0.001
        )
        ...
        for i in range(self.spokes_per_seg - 1):
            ...
            # solve for spoke
            spoke_arr[i + 1] = (
                np.cumsum(grad_cat[i + 1, :, self.points_before_curve :], axis=-1)
                * gamma_bar
                * self.dt_sampling
                * 0.001
            )
        ...
        grad_cat_flat = grad_cat_flat.reshape(
            3, self.spokes_per_seg * self.points_per_spoke
        )

        ## Rotate the gradients by segRots, then find spoke_arr
        spokes_full_arc = np.zeros(
            (
                nSegs_total * self.spokes_per_seg,
                3,
                self.points_per_spoke - self.points_before_curve,
            )
        )
        grads_full_arc = np.zeros(
            (nSegs_total * self.spokes_per_seg, 3, self.points_per_spoke)
        )
        ...
        for i in range(nSegs_total):
            ...
            if i == 0:  # don't rotate first segment
                ...
                for j in range(self.spokes_per_seg):
                    grads_full_arc[i * self.spokes_per_seg + j] = scale * (
                        M_file[i - 1].reshape(3, 3) @ grad_cat[j]
                    )
                    spokes_full_arc[i * self.spokes_per_seg + j] = (
                        np.cumsum(
                            grads_full_arc[
                                i * self.spokes_per_seg + j,
                                :,
                                self.points_before_curve :,
                            ],
                            axis=-1,
